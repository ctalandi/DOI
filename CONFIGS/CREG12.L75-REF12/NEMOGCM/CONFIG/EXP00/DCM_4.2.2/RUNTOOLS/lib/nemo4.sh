...
# To output restarts at mid-year and at the end of the year 
# DO NOT consider leap-year 
if [ $MULTIRST = 1 ] ; then
	# to save after 181 days (30th june) and at the end of the year
        rdt=$(LookInNamelist rn_Dt)
        midyear=` echo 1 | awk "{ b=$rdt ; a=181*86400/b ; print a }" `
        nitrst1=$(( nit000 + midyear -1 ))
        nitrst2=$nitend
else
        nitrst1=0
        nitrst2=0
...
sed -e "s/<NN_NO>/$no/" \
    ...
    -e "s/<NITRST1>/$nitrst1/" \
    -e "s/<NITRST2>/$nitrst2/" \
    -e "s/<OCERST_IN>/restart-$prev_ext/" \
    -e "s/<OCERST_OUT>/restart/" \
    -e "s@<CN_DIAOBS>@$DDIR/${CN_DIAOBS}.$no@"   \
    -e "s@<CN_DIRICB>@$DDIR/${CN_DIRICB}.$no@"   \
    -e "s@<CN_IN_OCERST>@$DDIR/${CN_DIRRST}.$prev_ext@"   \
    -e "s@<CN_OUT_OCERST>@$DDIR/${CN_DIRRST}.$no@"  namelist > znamelist1
...
if [ $ICE != 0 ] ; then
    ...
    sed -e "s/<ICERST_IN>/restart_ice-$prev_ext/" \
        -e "s/<ICERST_OUT>/restart_ice/" \
        -e "s@<CN_IN_ICERST>@$DDIR/${CN_DIRRST}.$prev_ext@"   \
        -e "s@<CN_OUT_ICERST>@$DDIR/${CN_DIRRST}.$no@"  namelist_ice > ztmp
...
case $STOP_FLAG in
    ...
    echo ' ==============================='
    ##CT if [ $RST_READY = 1 ] ; then
    ##CT    echo "   *** Restart files are ready from NEMO ..."
    ##CT else
    for member in  $(seq $ENSEMBLE_START $ENSEMBLE_END) ; do
        mmm=$(getmember_extension $member)
        nnn=$(getmember_extension $member nodot)
