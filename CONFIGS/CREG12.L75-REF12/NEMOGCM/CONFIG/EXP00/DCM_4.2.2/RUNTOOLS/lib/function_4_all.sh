...
#         getweight $blk $P_WEI_DIR $F_WEI_DIR
     ...
     if [ $tmp = T ] ; then blk=namsbc_rnf ;  getfiles $blk $P_RNF_DIR $F_DTA_DIR ;  fi
...
#          rstout = <CONFIG_CASE><.MBR>_<nitend>_<CN_RST_OUT>_<CORE>.<FILEXT>
 ...
 for rest in ${CONFIG_CASE}${mmm}_${znitend}_${2}_[[:digit:]]*[[:digit:]].${filext} ; do
     if [ $RST_READY = 1 ] ; then
       ...
       rest_in=${2}-${ext}${mmm}_${CORE}.${filext}
...
mk_batch_hdr --name ${1%%.*} --par --wallclock 0:20:00 --cores $ENSEMBLE_SIZE --cluster hpt --account $ACCOUNT --adapp --queue xlarge  > $1
...
# this script is valid for both ensemble and non ensemble run
  ...
  mk_batch_hdr --name ${1%%.*} --cores $NB_NPROC_MER --wallclock $WALL_CLK_MER \
             --account $ACCOUNT --cluster hpt  --adapp --queue xlarge \
      ...
      . $RUNTOOLS/lib/function_4_all.sh
         ...
         if [ \$? = 0 ] ; then
            ...
            cd SCALAR
              for f in *scalar*_0000.nc ; do
                 ...
                 ( 'OCE' ) filext='Ocescalar' ;;
                 ( 'ICE' ) filext='Icescalar' ;;
         ...
         ln -sf $MERGE_EXEC ./
         if [ \${mergeprog} == "mergefile4.exe" ]   ; then
             ./\$mergeprog -F -c $CN_DOMCFG -r
         else
...
update_db_file()  {
   ...
   if [ $nn_leapy = 1 ]  ;  then 
	# Take into account possible leap year
        if [ $ndays = 365 ] ; then
           znxty=$(( ${aammdd:0:4} + 1 ))
	...
	# Force a 1 year segment to have 365 days whatever the considered year
        dif=$(( 365 * $nstep_per_day ))
   fi
