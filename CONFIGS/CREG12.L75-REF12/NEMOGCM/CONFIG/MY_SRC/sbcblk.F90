...
!CT reduce wind intensity      
      INTEGER  ::   jmin, jmax, iloc, jloc   ! dummy loop indices
      REAL(wp) ::   zloc1, zloc2, zloc3, zloc4, zloc5
      REAL(wp), DIMENSION(jpi,jpj) ::   zwind_i, zwind_j    ! temporary 10m-wind components
!CT reduce wind intensity      
      !!----------------------------------------------------------------------
      !
...
!CT reduce wind intensity      
      ...
      ln_wndred =.FALSE.
!CT      zwind_i(:,:)=0.e0   ;   zwind_j(:,:)=0.e0
!CT      zwind_i(:,:)=sf(jp_wndi)%fnow(:,:,1)  ;   zwind_j(:,:)=sf(jp_wndj)%fnow(:,:,1)
!CT
!CT      zloc1=0.e0    ; zloc2=0.e0  ; zloc3=0.e0  ;  zloc4=0.e0  ; zloc5=0.e0 
!CT
!CT      iloc=mi0(166)  ;  jloc=mj0(1305) 
!CT      zloc1 = ABS(sf(jp_wndj)%fnow(iloc, jloc,1))
!CT
!CT      iloc=mi0(177)  ;  jloc=mj0(1265) 
!CT      zloc2 = ABS(sf(jp_wndj)%fnow(iloc, jloc,1))
!CT
!CT      iloc=mi0(189)  ;  jloc=mj0(1245) 
!CT      zloc3 = ABS(sf(jp_wndj)%fnow(iloc, jloc,1))
!CT
!CT      iloc=mi0(189)  ;  jloc=mj0(1225) 
!CT      zloc4 = ABS(sf(jp_wndj)%fnow(iloc, jloc,1))
!CT
!CT      iloc=mi0(177)  ;  jloc=mj0(1283) 
!CT      zloc5 = ABS(sf(jp_wndj)%fnow(iloc, jloc,1))
!CT
!CT      WRITE(numout,*) 'Wind intensities:', kt,  zloc1, zloc2, zloc3, zloc4, zloc5
!CT
!CT      !IF ( ( zloc1 >= 12. ) .OR. ( zloc2 >= 12. ) .OR. ( zloc3 >= 12. ) .OR. ( zloc4 >= 12. ) .OR. ( zloc5 >= 12. ) ) THEN
!CT      !        ln_wndred =.TRUE.
!CT      !ENDIF
!CT
!CT      !IF ( ln_wndred ) THEN 
!CT      IF ( kt >= 356440 ) THEN
!CT      
!CT            !IF( MOD( kt, 10000 ) == 0 .AND. lwp ) THEN
!CT                     WRITE(numout,*) '          Reduce the wind intensity close to the ORCA singularity, kt :', kt
!CT                     WRITE(numout,*) '                    BEFORE the increase:' , sf(jp_wndj)%fnow( mi0(186):mi1(190) , mj0(1315):mj1(1320) , 1 )
!CT            !ENDIF
!CT            
!CT
!CT            ! All the following is located south to the Victoria island from west to east 
!CT            jmin=1286 ; jmax=1325
!CT            sf(jp_wndi)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_i( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.80
!CT            sf(jp_wndj)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_j( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.80
!CT            
!CT            jmin=1245 ; jmax=1285
!CT            sf(jp_wndi)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_i( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.70
!CT            sf(jp_wndj)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_j( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.70
!CT            
!CT            jmin=1204 ; jmax=1244
!CT            sf(jp_wndi)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_i( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.70
!CT            sf(jp_wndj)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_j( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.70
!CT
!CT            jmin=1163 ; jmax=1203
!CT            sf(jp_wndi)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_i( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.80
!CT            sf(jp_wndj)%fnow( mi0(135):mi1(210), mj0(jmin):mj1(jmax), 1 ) = zwind_j( mi0(135):mi1(210), mj0(jmin):mj1(jmax) ) * 0.80
!CT
!CT
...
!CT            
!CT            !IF( MOD( kt, 10000 ) == 0 .AND. lwp ) THEN
!CT                     WRITE(numout,*) '                    AFTER the increase:' , sf(jp_wndj)%fnow( mi0(186):mi1(190) , mj0(1315):mj1(1320) , 1 )
!CT            !ENDIF
!CT
!CT      ENDIF
!CT
!CT      CALL FLUSH(numout)
