...
CONTAINS
   ...
   SUBROUTINE dom_msk( k_top, k_bot )
      ...
      INTEGER  ::   iktop, ikbot   !   -       -
      INTEGER  ::   ios, inum
!CT Mask few open water closed areas {
...
!CT Mask few open water closed areas { 
      IF(lwp) THEN
         WRITE(numout,*)
         WRITE(numout,*) ' >>>>>>>>>>>>>>>>>>>>>              CAUTION: Mask 2 grid points along Bering straithand before correction of the bathy file'
         WRITE(numout,*)
      ENDIF

      ! The following grid points are located on the northern BDY 
      ii0 = 644    ;   ii1 = 646
      ij0 = 1801   ;   ij1 = 1801   ;   tmask( mi0(ii0):mi1(ii1),mj0(ij0):mj1(ij1),:) = 0._wp
